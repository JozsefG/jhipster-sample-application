...
public interface XGroupRepository extends JpaRepository<XGroup, Long> {
    ...
    @Query(value = "select distinct x_group from XGroup x_group left join fetch x_group.users",
    ...
    @Query("select x_group from XGroup x_group left join fetch x_group.users where x_group.id =:id")
